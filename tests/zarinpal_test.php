...
/**
 * zarinpal enrolment plugin tests.
 *
...
class enrol_zarinpal_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['zarinpal'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['zarinpal']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('zarinpal'));
        $plugin = enrol_get_plugin('zarinpal');
        $this->assertInstanceOf('enrol_zarinpal_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_zarinpal', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $zarinpalplugin = enrol_get_plugin('zarinpal');

        // Just make sure the sync does not throw any errors when nothing to do.
        $zarinpalplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_zarinpal_plugin $zarinpalplugin  */
        $zarinpalplugin = enrol_get_plugin('zarinpal');
        ...
        $zarinpalplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $zarinpalplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $zarinpalplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $zarinpalplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $zarinpalplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $zarinpalplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $zarinpalplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $zarinpalplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $zarinpalplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $zarinpalplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $zarinpalplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $zarinpalplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $zarinpalplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
